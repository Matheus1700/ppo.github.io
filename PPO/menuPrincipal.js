class menuPrincipal extends Phaser.Scene{
    ...
    create(){
        ...
        this.add.dom(490, 338, 'div', 'background-color: white; width: 220px; height: 100px; font: 48px Arial', 'Phaser');


        let botaoCriar = this.add.image(664.5, 270, "botaoCriar1").setScale(0.60).
        setInteractive().on('pointerdown', () => { this.scene.start("criarPartida")});

        let botaoEstatistica = this.add.image(664.5, 430, "botaoEstatistica1").setScale(0.6)
        .setInteractive().on('pointerdown', () => { this.scene.start("estatisticas")});

        let botaoIniciar = this.add.image(664.5, 645, "botaoIniciar1").setScale(0.6)
        .setInteractive().on('pointerdown', () => { this.scene.start("entrarPartida")});

        let botaoFechar = this.add.image(664.5, 595, "botaoFechar1").setScale(0.6)
        ...
        this.texto1 = this.add.bitmapText(450, 214, "gamma", 'CRIAR PARTIDA');
        this.texto1.setScale(1.5);

        this.texto2 = this.add.bitmapText(452, 296, "gamma", 'ESTATISTICAS');
        this.texto2.setScale(1.5);

        this.texto3 = this.add.bitmapText(452, 376, "gamma", 'BUSCAR PARTIDA');
        this.texto3.setScale(1.5);

        this.texto4 = this.add.bitmapText(452, 461, "gamma", 'SAIR');
        ...
        botaoCriar.on("pointerover", ()=>{
            ...
            console.log("pointerOver funciona e muito bem :)");
        })

        botaoCriar.on("pointerout", ()=>{
            botaoCriar = ("botaoCriar1");
            console.log("pointerOut funciona e muito bem :)");
